...
for tool in python3 sha256sum timeout "${qemu_bin}"; do
...
assert type(info["schemaVersion"]) is int and info["schemaVersion"] in (1, 2)
...
assert info["validationOutcome"] in ("browser-bootable", "architecture-blocked", "browser-runtime-blocked", "qemu-unvalidated")
...
if info["bootable"]:
    ...
    assert info["validationOutcome"] in ("browser-bootable", "browser-runtime-blocked")
if info["schemaVersion"] == 2:
    assert type(info.get("browserCompatible")) is bool
    assert type(info.get("bootableCandidate")) is bool
    assert isinstance(info.get("capabilities"), dict)
...
print(str(info.get("browserCompatible", info["v86Compatible"])).lower())
print(str(info["bootSuccessMarkerImplemented"]).lower())
print(info["qemuBootMode"] or "none")
print(info["sha256"])
print(str(info.get("bootableCandidate", False)).lower())
PY
)
...
if [[ "${FL_REQUIRE_BROWSER_COMPATIBLE:-0}" == "1" && "${browser_compatible}" != "true" ]]; then
    echo "test-browser-kernel: browser-incompatible candidate cannot be promoted" >&2
    exit 1
fi
...
if [[ "${bootable}" == "true" || "${bootable_candidate}" == "true" ]]; then
    [[ "${marker_implemented}" == "true" ]] || {
        echo "test-browser-kernel: bootable candidate does not declare the marker" >&2
        exit 1
    }
    ...
    for tool in file readelf; do
        command -v "${tool}" >/dev/null 2>&1 || {
            echo "test-browser-kernel: required audit tool not found: ${tool}" >&2
            exit 1
        }
    done
...
if [[ "${bootable}" == "true" || "${bootable_candidate}" == "true" ]]; then
    if ! tr -d '\r' <"${qemu_log}" | grep -Fxq 'FLINTSTONE_KERNEL_BOOT_OK'; then
        cat "${qemu_log}" >&2
...
info['bootable']=True
info['validationOutcome']='browser-runtime-blocked'
info['blockers']=[x for x in info['blockers'] if not x.startswith('QEMU has not')]
with open(p,'w',encoding='utf-8') as f: json.dump(info,f,indent=2); f.write('\n')
PY
    fi
    echo "test-browser-kernel: PASS"
    echo "  QEMU boot probe: observed FLINTSTONE_KERNEL_BOOT_OK"
    echo "  boot signals: bootable=true, marker observed=true"
    if [[ "${browser_compatible}" != "true" ]]; then
        echo "  promotion: blocked until the declared browser emulator passes the same boot probe"
    fi
    exit 0
fi

if [[ ${qemu_status} -eq 0 ]] || tr -d '\r' <"${qemu_log}" | grep -Fxq 'FLINTSTONE_KERNEL_BOOT_OK'; then
    cat "${qemu_log}" >&2
