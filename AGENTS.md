...
- **`PRERELEASE`** — **`0`** or **`1`**; **`1`** = prerelease row that **must** live under **`version/entries/preproduction A.B.C/`** (see **`docs/versioning.md`**).
...
- **`GM`** — **`0`** or **`1`** (go-to-main). **`GM=1`** is allowed **only** under **`preproduction A.B.C/`** and at most once per directory. Before **`main`**, run **`./scripts/promote_preproduction_for_main.sh`**, which merges **all** prerelease **`.ver`** rows in that folder (sorted by **`DEV_VERSION`**) into one root GA file, **drops** **`PRERELEASE`**, **`GM`**, and **`DEV_VERSION`**, and **removes** the **`preproduction *`** directory from **both** **`version/entries`** and **`version/locked`**. **`main`** must have no **`preproduction *`** dirs and no **`PRERELEASE=1`**, **`GM=1`**, or **`DEV_VERSION=`** lines — CI **`check_version_main_prerelease_policy.sh`**.
