...
fl_result_t fl_net_resolve_ipv4(const char *host, uint32_t *out_addr_be, char *resolved_ip,
    ...
    if (strcmp(host, "localhost") == 0) {
        *out_addr_be = fl_net_htonl(0x7F000001u);
...
static fl_result_t dns_query_aaaa_once(const char *host, uint32_t ns_be, uint16_t txid,
                                       uint16_t sport_host, uint8_t out6[16]) {
    uint8_t query[256];
    uint8_t answer[576];
    size_t qlen;
    size_t alen;
...

    {
        ...
        rdlen = (uint16_t)(((uint16_t)answer[off] << 8) | answer[off + 1]);
        off += 2;
        if (rtype != 28u || rclass != 1u || rdlen != 16u || off + 16 > alen)
            return FL_RESULT_ERR;
        memcpy(out6, answer + off, 16);
        return FL_RESULT_OK;
    }
}
