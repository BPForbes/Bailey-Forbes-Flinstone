...
- **Cursor / AI-authored feature PRs:** commit **`version/entries/*.ver`** (and keep **`version/entries/ABOUT.txt`** byte-identical to the merge target’s **`version/locked/ABOUT.txt`** while **`version/locked/`** is unchanged—CI `check_version_locked_subset_of_entries.sh`). **Do not** commit **`version/locked/**`** in those PRs: **Version lock on merge** (GitHub Actions after push to **`develop`**) runs **`relocate_root_prerelease_ver_to_preproduction.sh`**, **`finalize_version_locked.sh`**, stamps dates when applicable, then **`gen_version_def.sh`** via the same relocate+generator path as **`.github/actions/apply-version-def-from-ver-trees`**, and opens a PR to publish **`version/locked/`** and the header when needed. **Do not hand-edit** **`VERSION_*`** or **`VERSION_LINE`** in **`userland/shell/version_def.h`** when **`GM=1`** changes the effective semver—commit **`.ver`** only and let automation regenerate the header. **CI** may push **`version/entries`** + **`userland/shell/version_def.h`** on same-repo pushes when needed. Fork PRs and offline checks follow **`docs/versioning.md`**. **Maintainers** may run finalize locally before merge if they want an early snapshot on the branch.
