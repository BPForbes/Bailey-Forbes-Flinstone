...
2. Set **`MAJOR_VERSION`**, **`STANDARD_VERSION`**, **`RELEASE_VERSION`**, and **`DESCRIPTION`** (single line or `DESCRIPTION<<DELIM` … `DELIM` heredoc). See **`version/entries/ABOUT.txt`** for syntax.
...
- **`AGENTS.md`** — Cursor Cloud, tests, and versioning summary (including pre-merge **CodeRabbit** and **Codex CLI** policy).
