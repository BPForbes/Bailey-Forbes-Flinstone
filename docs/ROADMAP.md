...
**Patch-scale TODOs (`~` prefix on integration ✅):** When a row’s **module integration** is **~✅**, hosted/lab wiring and tests are in place and any open work is tracked only as **minor** in-source comments (**`TODO(P*/Codex)`**, **`TODO(CR)`**) or phase follow-ups (e.g. **TODO: P2-3** kernel-path authz) suitable for a **patch**, not a missing subsystem. Those markers are **ignored** for contract-completion (**✅** stays **✅**). Inventory and file paths: **`docs/p0_p2_pr_coverage.md`** (section *Code TODO markers*). **P0–P2 integration snapshot:** every row is **✅** or **~✅** as of the **4.1.0** prerelease train—no **⚠️** in that band.
