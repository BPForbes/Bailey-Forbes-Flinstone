#!/usr/bin/env bash
# Main-line policy: no preproduction/* directories; no PRERELEASE=1, GM=1, or DEV_VERSION=
# in any .ver under version/entries or version/locked. Run promote_preproduction_for_main.sh
# before merging develop → main so preproduction trees are merged to a single root GA .ver
# and removed from both entries and locked.
set -euo pipefail
...
if find "$ROOT/version/entries" "$ROOT/version/locked" -mindepth 1 -type d -name 'preproduction *' -print -quit 2>/dev/null | grep -q .; then
  ...
  echo "  Run: ./scripts/promote_preproduction_for_main.sh (merges GM=1 preproduction dirs, deletes them from entries and locked)" >&2
  err=1
fi
...
while IFS= read -r -d '' f; do
  ...
  if grep -qE '^[[:space:]]*(int[[:space:]]+)?DEV_VERSION=' "$f"; then
    echo "check_version_main_prerelease_policy: DEV_VERSION must not appear in $f on main (ship GA rows only)." >&2
    err=1
  fi
