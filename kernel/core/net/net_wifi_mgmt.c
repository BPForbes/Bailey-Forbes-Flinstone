...
static void mgmt_write_hdr(uint8_t *out, uint16_t fc, const uint8_t addr1[6],
    ...
    fl_net_put_u16_le(out, fc);
    fl_net_put_u16_le(out + 2, 0u);
    asm_mem_copy(out + 4, addr1, 6u);
    asm_mem_copy(out + 10, addr2, 6u);
    asm_mem_copy(out + 16, addr3, 6u);
    fl_net_put_u16_le(out + 22, 0u);
}
...
fl_result_t fl_net_wifi_mgmt_parse_auth_resp(const uint8_t *frame, size_t len,
    ...
    if (auth_alg_out)
        *auth_alg_out = fl_net_get_u16_le(frame + 24);
    if (auth_seq_out)
        *auth_seq_out = fl_net_get_u16_le(frame + 26);
    if (status_out)
        *status_out = fl_net_get_u16_le(frame + 28);
    if (body_out)
...
fl_result_t fl_net_wifi_mgmt_build_auth_req(const uint8_t sta_mac[6], const uint8_t bssid[6],
    ...
    fl_net_put_u16_le(out + 24, 0u);
    fl_net_put_u16_le(out + 26, 1u);
    fl_net_put_u16_le(out + 28, 0u);
...
fl_result_t fl_net_wifi_mgmt_build_auth_resp(const uint8_t bssid[6], const uint8_t sta_mac[6],
    ...
    fl_net_put_u16_le(out + 24, 0u);
    fl_net_put_u16_le(out + 26, auth_seq);
    fl_net_put_u16_le(out + 28, 0u);
    *out_len = FL_WIFI_MGMT_HDR_LEN + 6u;
    return FL_RESULT_OK;
}
...
fl_result_t fl_net_wifi_mgmt_build_twt_setup_req(const uint8_t sta_mac[6], const uint8_t bssid[6],
                                                 ...
                                                 uint8_t *out, size_t out_cap, size_t *out_len) {
    size_t need = FL_WIFI_MGMT_HDR_LEN + 3u + 2u + FL_WIFI_TWT_ELEM_LEN;
...
fl_result_t fl_net_wifi_mgmt_parse_twt_setup_resp(const uint8_t *frame, size_t len,
    ...
    if (mgmt_fc(frame) != FL_WIFI_MGMT_FC_ACTION)
        return FL_RESULT_INVAL;
    if (len < FL_WIFI_MGMT_HDR_LEN + 3u + 2u + FL_WIFI_TWT_ELEM_LEN)
        return FL_RESULT_INVAL;
    if (frame[24] != FL_WIFI_ACTION_CAT_S1G || frame[25] != FL_WIFI_ACTION_TWT_SETUP)
        return FL_RESULT_INVAL;
