...
- **`userland/shell/version_def.h` and `DEV_VERSION` (AI):** **Never** stage or commit **`userland/shell/version_def.h`** on same-repo feature PRs; **`make`** may refresh it locally—leave it unstaged or match the merge target. **Do not** bump **`DEV_VERSION`** mid-PR.
