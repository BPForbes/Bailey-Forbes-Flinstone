...
- **Optional prerelease layout:** **`PRERELEASE=1`** rows live under **`version/entries/preproduction A.B.C/`**; bump **`PRERELEASE_ITER`** with **`./scripts/bump_prerelease_iter.sh`**. Before any merge that lands **`version/**`** on **`main`**, run **`./scripts/promote_preproduction_for_main.sh`** (or **`make promote-preproduction-for-main`**); **`main`** must not ship **`preproduction *`** or **`PRERELEASE=1`** — CI fails otherwise.
