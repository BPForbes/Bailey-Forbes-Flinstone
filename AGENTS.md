...
- **`PRERELEASE_ITER`** — non-negative int; with **`PRERELEASE=1`** use **`>= 1`** and bump with **`./scripts/bump_prerelease_iter.sh`**. On promotion to GA, keep the last iter and set **`PRERELEASE=0`** (**`make promote-preproduction-for-main`**). **`main`** must have no **`preproduction *`** dirs and no **`PRERELEASE=1`** — CI **`check_version_main_prerelease_policy.sh`**.
