...
| **`DEV_VERSION`** | Non-negative integer. With **`PRERELEASE=1`**, use **`>= 1`**. Humans may bump per iteration using **`./scripts/bump_dev_version.sh`** when policy calls for it. **Cursor / AI on same-repo feature PRs:** set **`DEV_VERSION`** once when the row is created; **do not** bump it on every push in the same PR. You may keep **`DEV_VERSION`** on a **root** row while **`PRERELEASE`** is still **`0`** to record develop-side iteration **before** moving the line under **`preproduction/`**. **`main`** must ship **`.ver`** files **without** any **`DEV_VERSION=`** line (CI). |
