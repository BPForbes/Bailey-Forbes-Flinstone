...
/** Compose landing basename from offer file_name (transfer id lives in sidecar only). */
fl_result_t fl_server_shared_landed_basename(const fl_server_file_offer_t *offer,
                                             char *out,
                                             size_t out_cap);

/** Save bytes to server_shared/<file_name> (no on-disk sidecar). */
...
/** Session-only expiry; disk purge deferred until host-side index exists. */
