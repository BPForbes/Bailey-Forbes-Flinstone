...
Issue **#328** end-to-end (WSL and native Linux): `./scripts/validate_issue_328.sh --help` then `./scripts/validate_issue_328.sh --yes`. The script installs missing packages, applies idempotent DHCP/echo firewall rules (Linux sysctl/iptables/nft plus Windows/Hyper-V via `powershell.exe` on WSL), runs `make test_p3_network` (required), lab/TWT unit tests, and optional `mac80211_hwsim` + in-tree `fl_net_wifi_connect` (not nmcli/wpa_cli). On WSL, **SKIP** for hwsim / uart-hw is expected unless USB Wi-Fi (`usbipd`) or an ESP32 is attached; Microsoft kernels do not provide `linux-modules-extra-$(uname -r)` via apt. Windows Wi-Fi is not `wlan0` inside WSL — physical OTA needs an nl80211 STA from `iw dev`.
