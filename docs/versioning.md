...
1. On the **first** substantive code or docs change for a pull request, add **`version/entries/<A>_<B>_<C>_<short_slug>.ver`** (or additional rows under **`preproduction <A>.<B>.<C>/`**) if nothing yet covers that PR. **AI/automation:** the **preproduction version** is **A.B.C** read from **`version/entries/preproduction <A>.<B>.<C>/`** directory names (**`PRERELEASE=1`** rows belong there)—do **not** add a **new root** **`.ver`** at **`version/entries/`** whose **A.B.C** is **numerically below** that **A.B.C** unless a **maintainer explicitly** requests that separate GA release. Track ongoing work with **`DEV_VERSION`** or additional **`*.ver`** files in the **same** preproduction directory. For a **single GA feature** at the entries root without **`preproduction */`**, prefer **one** **`.ver`** revised in place.
