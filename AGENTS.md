...
**Versioning (merged AI branches):** after **`git merge --no-ff`**, **delete** the merged-in branch’s duplicate **`.ver`** for the same **A.B.C** train. **Do not edit** immutable **`.ver`** paths from the merge base—fold combined release prose into the single PR-owned **`.ver`**. Only add a new root **`.ver`** with the next **`DEV_VERSION`** when the combined PR does not already have one. Re-run **`./scripts/check_version_entries_semver_dev_unique.sh`**.

**Immutable `.ver` rows:** any **`version/entries/**/*.ver`** on the merge base must not be edited. Before adding a **`.ver`**, inspect the current submitted PR/branch for an existing PR-owned **`.ver`** (including rows CI relocated under **`version/entries/preproduction */`**). If one already exists, update that file's **`DESCRIPTION`** only; do **not** add another. If the PR-owned row was already relocated under **`version/entries/preproduction */`** and you change its **`DESCRIPTION`**, move that same file back to the **`version/entries/`** root and remove any stamped **`RELEASE_DATE`** so the version job can relocate and stamp it again. Each PR should carry **exactly one new** **`.ver`** total unless a human maintainer explicitly requests a separate release entry. **`GM=1`** go-to-main rows use **`DESCRIPTION`** starting with **`A.B.C:`** and an overall release summary (see **`docs/versioning.md`**).
...
- **`PUBLISHED_DESCRIPTION`** — optional, single line only (no heredoc). **Legal only alongside `GM=1`** on the same row — see the dedicated subsection below for the full ruleset (length cap, and the fidelity-to-`DESCRIPTION` check CodeRabbit is asked to perform on every PR that touches it).
...
**AI assistants:** When you begin the **first** substantive code change for a pull request, first check whether the current submitted PR/branch already has a PR-owned **`.ver`** (for example with **`git diff --name-only <merge-base>...HEAD -- version/entries`**, and also by looking for CI-relocated rows under **`version/entries/preproduction */`**). If one exists, **extend that row's `DESCRIPTION` only**; do **not** add another. If it was already relocated under **`preproduction */`**, move that same file back to the **`version/entries/`** root and remove any stamped **`RELEASE_DATE`** before submitting so the version job can relocate/stamp it again. If none exists, **add exactly one** new root **`version/entries/*.ver`** so the work is documented. **Unique `(MAJOR, STANDARD, RELEASE, DEV_VERSION)`** across **`version/entries/**/*.ver`** (missing **`DEV_VERSION=`** counts as **0**): scan existing **`.ver`** files for that **A.B.C** and pick the next free **`DEV_VERSION`**; CI runs **`./scripts/check_version_entries_semver_dev_unique.sh`**. For **`PRERELEASE=1`**, **author new rows at the `version/entries/` root only** on same-repo branches—**do not** hand-add new **`*.ver`** paths under **`preproduction <A>.<B>.<C>/`** (Actions moves root rows into that directory); **do not** add new files under **`version/locked/`**; **omit `RELEASE_DATE=`** on new rows (Actions dates on relocate / Version lock). See **`version/entries/ABOUT.txt`** (AI authoring). Fork PRs follow **`docs/versioning.md`** (relocate locally, then commit). **Do not backtrack semver against an active preproduction version:** the **preproduction version** is **A.B.C** from **`version/entries/preproduction <A>.<B>.<C>/`** after relocate (**`PRERELEASE=1`** rows land there); do **not** add a **new root** **`version/entries/*.ver`** whose **A.B.C** is **numerically below** that **A.B.C** unless a **human maintainer** explicitly asks for that separate shipped release. For a **single GA feature** at **`version/entries/`** root without **`preproduction */`**, revise the PR's single **`.ver`** in place. **Do not** commit **`version/locked/**`** on agent-authored feature PRs. **Do not** commit **`userland/shell/version_def.h`**; let **CI** or **Version lock on merge** publish it (see **AI — `userland/shell/version_def.h`, `DEV_VERSION`, and `GM`** above).

### `PUBLISHED_DESCRIPTION` (portfolio summary)

Full mechanics (promotion, `metadata/releases.json` upsert, worked example): **`docs/versioning.md`** § *`PUBLISHED_DESCRIPTION`*. This section is the **ruleset** — the three things a diff adding or editing `PUBLISHED_DESCRIPTION` must satisfy, and what happens when it does not.

1. **Configuration: `GM=1` only.** `PUBLISHED_DESCRIPTION` is legal **only** on a row that also sets `GM=1`. Never on a root `.ver` (`GM=1` is already forbidden there), never on a `PRERELEASE=1` row that is not the `GM=1` candidate in its `preproduction A.B.C/` folder, never with `GM=0` or `GM` absent. CI enforces this — **`./scripts/check_version_published_description_requires_gm.sh`**, wired into the **Versioning requirements** job — but do not rely on CI alone: an AI agent proposing a `.ver` change should verify this itself before opening the PR, the same way it never self-authorizes `GM=1` in the first place.
2. **Length: 100-character hard cap.** CI rejects anything longer (character count, computed in a locale-independent way — a `·` or `–` counts as one character). Treat 100 as a ceiling, not a target: real curated summaries run 30–70 characters (`"Wi-Fi · WPA2/WPA3-SAE · nl80211"`). If a draft is pushing up against the cap, that is a sign to compress harder, not to trim one word until it fits.
3. **Fidelity to `DESCRIPTION`: a judgment call, not a script.** `PUBLISHED_DESCRIPTION` must be a compressed, faithful restatement of that same row's `DESCRIPTION` — never an independent claim. Before writing one, re-read the row's `DESCRIPTION` and ask: does this short line contradict it, invent something it does not support (a number, a capability, a "first"/"latest" not actually claimed), or bury the release's actual headline behind a secondary detail? A shorter version of the same claim is correct; a different claim is not.

**Reviewers (CodeRabbit and humans):** `.coderabbit.yaml`'s `version/**/*.ver` path instructions ask CodeRabbit to check all three of the above on every PR touching `PUBLISHED_DESCRIPTION` and **leave a review comment naming the specific rule broken** (configuration, length, or fidelity) when one does not hold — not a generic "looks off," but which rule and why, so it is directly actionable. Treat a CodeRabbit comment citing this ruleset the same as any other correctness finding: fix it or explain why it does not apply, rather than dismissing it as style noise.
