...
typedef void (*fl_net_wifi_nl80211_mgmt_cb_t)(const uint8_t *frame, size_t len,
                                              void *ctx);
...
/** Register RX filter for a management frame type (NL80211_CMD_REGISTER_FRAME). */
...
/** Poll netlink and dequeue one received management frame (if any). */
