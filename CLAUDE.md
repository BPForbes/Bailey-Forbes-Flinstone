...
Implement the bump by adding **`version/entries/<A>_<B>_<C>_<slug>.ver`**. Pushing to **`develop`** triggers **Version lock on merge** (entries → **`version/locked/`**, regenerate **`version_def.h`**, opens a PR when needed). For a local release build after that, use **`make deploy`** or the **Deploy** workflow.
