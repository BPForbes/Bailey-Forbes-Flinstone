...
| **P3-10** | ~✅ | **`contract_p3_wifi.h`**, **`net_wifi_he`**, in-tree SAE/EAPOL/TWT OTA (**`wifi_connect_ota`**, **`wifi_twt_ota`**, **`test_wifi_connect_ota`**), **`fl_net_dhcp_acquire`** post-association on Wi‑Fi netdev, lab scan/connect (**`FL_NET_WIFI_LAB`**, **`LabWpa2`** WPA2-PSK without OS supplicant), physical nl80211 path (**`FL_NET_WIFI_IFACE`**), **`wifi_router`** DB + shell **`wifi`** verbs; **`mac80211_hwsim`** CI job (**`make test_wifi_hwsim`**, artifacts retained); ESP AT **`wifi scan` / `wifi join`** over PTY (**`test_wifi_uart_at_scan_join`**). **P4-01** (#328) remaining physical RF boxes (real-AP TWT / NIC DHCP-UDP when hwsim is absent) keep this row **~✅**, not plain **✅**. |
...
| **TODO: P3-13 (#279) / P4-01 (#328)** | Wi‑Fi 802.11ax station production tail (**P3-10** / **P4-01**): **~✅** in-tree OTA on PR **#334** plus the eight-task follow-up: hwsim CI OTA job, ESP AT PTY scan/join, WPA2-PSK without OS supplicant, TWT `flow_id` lab record, in-tree DHCP/UDP on the Wi‑Fi netdev, EAPOL 4-way mock + hwsim pcap decode. Do **not** mark this row plain **✅** while a real-AP RF run is still the missing evidence for TWT/DHCP/UDP on physical NIC. See **`docs/GITHUB_ISSUE_SYNC_P4_WIFI_OTA.md`**. |
...
| Wi‑Fi | **IEEE 802.11ax-2021** / **802.11i**; **WPA3-SAE**; **RFC 2131** (DHCP after link). **P3-10** contract **✅**; module **~✅** in-tree OTA + #328 eight-task CI/lab evidence — production RF tail still open. **`docs/GITHUB_ISSUE_SYNC_P4_WIFI_OTA.md`**. |
