...
| `iw`, `hostapd`, `kmod`, `dnsmasq-base` | Optional local `mac80211_hwsim` / hostapd lab AP for Wi-Fi OTA experiments |
| `iptables` / `nftables` | Optional Linux firewall rules (UDP 67/68/echo). WSL images often omit them. |
| `linux-modules-extra-$(uname -r)` | Optional `mac80211_hwsim` on Ubuntu/Debian **host** kernels. **Not** published for Microsoft WSL kernels (`*-microsoft-standard-WSL2`). |
