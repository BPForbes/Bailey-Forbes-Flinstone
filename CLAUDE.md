...
- **`GM=1` go-to-main (maintainer / explicit AI instruction):** add a **new** **`.ver`** with the next **`DEV_VERSION`** and **`GM=1`**; **`DESCRIPTION`** must start with **`A.B.C:`** (e.g. **`4.0.1:`**) and summarize the **overall** release. **`promote_preproduction_for_main.sh`** copies only that text to the GA root **`.ver`**.
- **`PRERELEASE` / `GM`:** binary flags — when present, values must be exactly **`0`** or **`1`** (enforced by **`scripts/lib/ver_field_parse.sh`** and CI).
- **CI date stamp:** **`.github/actions/prepare-version-entries`** runs **`relocate_root_prerelease_ver_to_preproduction.sh`** then **`stamp_version_entries_release_date.sh`** on the **entries root** and every **`preproduction <A>.<B>.<C>/`** tree before **`gen_version_def.sh`**.
