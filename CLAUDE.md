...
- **Optional `DEV_PHASE` in `.ver`:** develop-only phase **D** for the same **A.B.C** (not in **`VERSION`**). Bump with **`./scripts/bump_dev_phase_in_ver.sh`** or **`make bump-dev-phase-in-ver VER=…`**. Before any merge that lands **`version/**`** on **`main`**, strip **`DEV_PHASE`** (**`make strip-dev-phase-from-ver`**); **`main`** must not ship **`DEV_PHASE=`** lines—CI fails otherwise.
