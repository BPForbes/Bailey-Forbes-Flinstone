...
### 3.8 File transfer (native protocol)

The **native Flinstone server file transfer** is available; SFTP is a compatibility adapter over that native service, not the core file-transfer system. See **`docs/SERVER_FILE_TRANSFER_SFTP_PLAN.md`** for implementation notes.
...
## 8 — Module map (server foundation shipped; native file transfer available)
...
| **`userland/command/cmd_server_file.c`** | ✅ shipped | **`server file`** send/inbox/public/sent/accept/decline/revoke; **`-Ex`** expiration |
...
| **`kernel/core/net/net_file_delivery.c`** | ✅ shipped | **`FILE_OFFER` / `FILE_CHUNK` / `FILE_DONE`** plus accept/decline/revoke/list/status; **`server_shared/`** staging |
...
| File-chunk path (`server send -file`) | ✅ shipped | Legacy alias routed to **`server file -user`**; chunk transfer and disposition opcodes implemented |
