...
**One new `.ver` per PR (AI):** Before adding a **`.ver`**, inspect the current submitted PR/branch for an existing PR-owned **`.ver`** (for example **`git diff --name-only <merge-base>...HEAD -- version/entries`**, and also check for CI-relocated rows under **`version/entries/preproduction */`**). If one exists, record follow-up work by updating that file's **`DESCRIPTION`** only; **do not** add another **`.ver`**. If the PR-owned row is already under **`preproduction */`** and its **`DESCRIPTION`** changes, move that same file back to the **`version/entries/`** root and remove any stamped **`RELEASE_DATE`** so the version job can relocate and stamp it again. If none exists, record the PR's work in **exactly one** new **`version/entries/A_B_C_slug.ver`** at the entries root. **Do not** append unrelated prose or bump **`DEV_VERSION`** on **`.ver`** files already sitting under **`version/entries/preproduction */`** from another train—those rows are **that** train's relocated record.

**Combining AI/agent branches (one PR / one train):** When merging a side agent branch into the main feature branch, use **`git merge --no-ff <side-branch>`** so the combine point is visible. After the merge, **delete** duplicate **`.ver`** files for the same **A.B.C** train. **Do not edit** immutable **`.ver`** paths from the merge base (**`DESCRIPTION`**, **`DEV_VERSION`**, **`GM`**, **`PRERELEASE`**, or semver fields). Fold combined release prose into the single PR-owned **`.ver`**; only add a new root **`version/entries/A_B_C_slug.ver`** with the next **`DEV_VERSION`** when the combined PR does not already have one. Then run **`./scripts/check_version_entries_semver_dev_unique.sh`**. See **Immutable `.ver` rows** below. Authoritative checklist: **AGENTS.md** § *AI feature-branch workflow*; IDE rules **`.cursor/rules/versioning.mdc`**; review hints **`.coderabbit.yaml`**.
...
**Reviewers — `version_def.h` on feature PRs:** Inspect **all commits on the branch**, not only the latest diff. **Do not** request reverting **`VERSION_LINE`** / **`VERSION_*`** when the change is from **GitHub Actions** (**`github-actions[bot]`**, **`chore(version):`**, **`gen_version_def.sh`**, **`prepare-version-entries`**, **`sync entries promote, finalize locked, and version_def.h`**, or **Version lock on merge**). On mixed agent + CI branches, treat header churn as automation when it matches **`gen_version_def.sh`** output for the branch’s **`version/entries/**/*.ver`** trees (**`.coderabbit.yaml`**, **AGENTS.md**, **CLAUDE.md**).

1. On the **first** substantive code or docs change for a pull request, add **`version/entries/<A>_<B>_<C>_<short_slug>.ver`** at the **`version/entries/`** root only if the submitted PR does not already own a **`.ver`**. If the PR-owned row was already relocated under **`preproduction */`** and its **`DESCRIPTION`** changes, move that same file back to the entries root and remove **`RELEASE_DATE`** before submitting. For **`PRERELEASE=1`**, **do not** hand-add new **`.ver`** paths under **`preproduction <A>.<B>.<C>/`** on same-repo branches—author at the root; CI **`relocate_root_prerelease_ver_to_preproduction.sh`** moves them into that directory. **AI/automation:** the **preproduction version** is **A.B.C** read from **`version/entries/preproduction <A>.<B>.<C>/`** directory names (**`PRERELEASE=1`** rows land there **after** relocate)—do **not** add a **new root** **`.ver`** at **`version/entries/`** whose **A.B.C** is **numerically below** that **A.B.C** unless a **maintainer explicitly** requests that separate GA release. For a **single GA feature** at the entries root without **`preproduction */`**, prefer **one** **`.ver`** revised in place.
...
## Immutable **`.ver`** rows (add new files; do not edit history)

- Any **`version/entries/**/*.ver`** path that already exists on the **merge-base** branch is **immutable** on feature PRs: do **not** edit **`DESCRIPTION`**, **`DEV_VERSION`**, **`GM`**, **`PRERELEASE`**, or semver fields in that file.
- Each new PR scope, develop iteration, or go-to-main step: **add exactly one new** **`version/entries/A_B_C_slug.ver`** at the **entries root** only when the submitted PR does not already have a PR-owned **`.ver`**. If it does, update that row's **`DESCRIPTION`** only. If the row was already relocated under **`preproduction */`**, move it back to entries root and remove **`RELEASE_DATE`** before submission. For non-GM **`PRERELEASE=1`**, CI **`relocate_root_prerelease_ver_to_preproduction.sh`** moves the root row under **`preproduction <A>.<B>.<C>/`**.
- The **new** **`.ver`** your branch introduces may update **`DESCRIPTION` only** (no **`DEV_VERSION`** re-bump) until it merges to **`develop`**; after merge, that path is immutable too.
- **GA-oriented** work at the entries root without **`preproduction */`**: prefer **one** new root **`.ver`** per PR; do **not** revise older root rows from the merge base.
...
| Key | Meaning |
|-----|--------|
| **`PRERELEASE`** | **Binary:** **`0`** or **`1`** only when the key is present (CI rejects other values). **`0`** or omitted = GA-oriented row at the **`version/entries/`** root. **`1`** = prerelease: **author at the `version/entries/` root** on same-repo branches; **`relocate_root_prerelease_ver_to_preproduction.sh`** moves the file under **`preproduction <A>.<B>.<C>/`** (do **not** hand-add new **`PRERELEASE=1`** **`.ver`** paths under that directory on same-repo PRs). Fork PRs must relocate locally, then commit. |
...
| **`GM`** | **Binary:** **`0`** or **`1`** only when the key is present. **`0`** or omitted by default. **`1`** = maintainer signals this **`preproduction <A>.<B>.<C>/`** train is ready for **`promote_preproduction_for_main.sh`** to emit one GA root **`.ver`** at **`version/entries/`** using the **`GM=1`** file’s basename, semver, optional **`RELEASE_DATE`**, and **`DESCRIPTION`** (**allowed only inside** **`preproduction <A>.<B>.<C>/`**; at most **one** file per directory may set **`GM=1`**; set it on a **new** **`.ver`** row with the next **`DEV_VERSION`**, not by editing older train files). **Cursor / AI:** **never** set **`GM=1`** without **explicit human maintainer** permission. |
...
On **same-repo** pull requests into **`develop`** and on **feature-branch pushes** (not **`develop`** / **`main`** themselves), GitHub Actions may commit updates for you: **`.github/actions/prepare-version-entries`** runs **`relocate_root_prerelease_ver_to_preproduction.sh`**, **`promote_preproduction_for_main.sh --normalize-gm-only`**, **`promote_preproduction_for_main.sh`** (promotes when any **`preproduction */`** row has **`GM=1`**; no-op otherwise), then **`stamp_version_entries_release_date.sh`** (entries root **`*.ver`** plus every **`preproduction <A>.<B>.<C>/`** tree on the branch), before **`check_version_prerelease_layout.sh`**. The same workflow job then runs **`./scripts/gen_version_def.sh`** (which already reads **`version/entries/**/*.ver`**, including **`preproduction */`**) and may push one automated commit covering **`version/entries`** and **`userland/shell/version_def.h`** when needed. **Fork** pull requests cannot receive that push—run **`make prepare-version-entries`** (or the scripts in that order) and **`gen_version_def.sh`** locally, commit, and push. **`develop`** uses **Version lock on merge** to relocate and normalize before **`finalize_version_locked.sh`** and includes **`version/entries/**`** in the sync PR when paths change.

**Date stamp policy (feature CI):** **`relocate_root_prerelease_ver_to_preproduction.sh`** appends **`RELEASE_DATE=YYYY-MM-DD`** on root **`PRERELEASE=1`** rows before moving them into **`preproduction A.B.C/`**. **`stamp_version_entries_release_date.sh`** then fills any still-missing **`RELEASE_DATE`** on the **entries root** and on **active** **`preproduction <A>.<B>.<C>/`** directories (each top-level preproduction folder under **`version/entries/`**). Author new **`PRERELEASE=1`** rows at the **entries root**; do not rely on hand-placing rows only under **`preproduction */`** if you need the PR **Versioning requirements** job to date them.
...
### **`GM=1`** go-to-main **`DESCRIPTION`** (release prose)

The **`GM=1`** row is the final prerelease record before **`main`**. Its **`DESCRIPTION`** must start with **`A.B.C:`** (literal semver, e.g. **`4.0.1:`**) followed by the **overall release summary** for that version—all patches, features, and fixes in the train. **`promote_preproduction_for_main.sh`** copies **only** that text into the root GA **`.ver`** (not prose from other rows in the folder).
...
**Finalize before `main`:** a **maintainer** adds a **new** **`.ver`** under **`preproduction <A>.<B>.<C>/`** with **`GM=1`** (next **`DEV_VERSION`**) and an **`A.B.C:`** overall **`DESCRIPTION`**. **Same-repo CI** (**`prepare-version-entries`**, **Version lock on merge**) then runs **`./scripts/promote_preproduction_for_main.sh`** (or **`make promote-preproduction-for-main`** locally) after **`--normalize-gm-only`**. Add **`--dry-run`** to preview the GA root **`.ver`**, duplicate-GM normalization, and preproduction directory removal without writing files. For each such directory under **`version/entries`** and **`version/locked`** that contains one or more **`GM=1`** rows among its **`*.ver`** files, the script selects the highest **`DEV_VERSION`** GM row, rewrites lower **`DEV_VERSION`** GM rows to **`GM=0`**, then writes **one** new root GA **`.ver`** under **`version/entries/`** using the selected file’s **basename**. **`MAJOR_VERSION`**, **`STANDARD_VERSION`**, **`RELEASE_VERSION`**, optional **`RELEASE_DATE`**, and **`DESCRIPTION`** are taken **only** from that selected **`GM=1`** file; the shipped GA row **omits** **`PRERELEASE`**, **`GM`**, and **`DEV_VERSION`** entirely. It then **removes every** **`*.ver`** in that **`preproduction A.B.C/`** folder and **deletes** the directory from **`version/entries`** (and from **`version/locked`** only if a legacy copy existed there). Other prerelease rows in that folder are **not** merged into prose—they remain in git history only. Use **`--normalize-gm-only`** to demote lower **`DEV_VERSION`** duplicate GM rows without promoting. **`finalize_version_locked.sh`** never copies **`preproduction */`** into **`locked`**, so after promotion the next finalize copies **only** that new root **`.ver`** (and the rest of **`version/entries/`** without **`preproduction */`**) into **`version/locked/`**. Directories with **`PRERELEASE=1`** but **no** **`GM=1`** yet are left untouched.
...
| **`PUBLISHED_DESCRIPTION`** over **100 characters** | invalid (CI rejects) |

**Length: 100-character hard cap, CI-enforced.** **`scripts/check_version_published_description_requires_gm.sh`** rejects any **`PUBLISHED_DESCRIPTION`** longer than **`PUBLISHED_DESCRIPTION_MAX_LEN`** (**`100`**, defined once in that script — grep it there rather than trusting the number to stay in sync across docs) characters. The count is **characters**, not bytes: a middle dot (**`·`**) or en dash (**`–`**) is one character even though it is multiple UTF-8 bytes, and the check computes length in Python specifically so it does not depend on the CI runner's locale. **100** is a hard ceiling, not a target — every summary curated so far runs **30–70** characters (`"Wi-Fi · WPA2/WPA3-SAE · nl80211"`, `"Inheritable contracts · IPC/VFS hardening · shell improvements"`); a value that only just fits under the cap is a sign it is trying to be a second **`DESCRIPTION`** rather than a compact pointer to one.

**Fidelity to `DESCRIPTION`: reviewed, not scripted.** Whether **`PUBLISHED_DESCRIPTION`** actually and faithfully summarizes that same row's **`DESCRIPTION`** — no contradiction, no invented claim, no dropped headline point — cannot be checked by a shell script; **`.coderabbit.yaml`**'s **`version/**/*.ver`** path instructions ask CodeRabbit to read both fields together on every PR that touches **`PUBLISHED_DESCRIPTION`** and leave a review comment naming the specific rule broken (configuration, length, or fidelity) when one does not hold. AI agents authoring a **`PUBLISHED_DESCRIPTION`** should hold themselves to the same three checks before opening the PR — see **AGENTS.md** § *`PUBLISHED_DESCRIPTION` (portfolio summary)*.
...
**Preproduction intake (relocate):** **`./scripts/relocate_root_prerelease_ver_to_preproduction.sh`** (or **`make relocate-root-prerelease-ver`**) runs in CI before prerelease layout checks. For each **`version/entries/*.ver`** at the repository root with **`PRERELEASE=1`** and without **`GM=1`**, if **`RELEASE_DATE`** is absent it appends **`RELEASE_DATE=`** using **that run’s calendar day** while the file is still at the root, **then** moves the file under **`preproduction <A>.<B>.<C>/`**. Root **`GM=1`** rows are left in place for promotion rather than relocated down.
