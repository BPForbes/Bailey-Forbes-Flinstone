...
- **`DEV_PHASE`** — optional positive integer (**develop phase D**) for the same **A.B.C**; omitted means phase **1** for changelog ordering; **never** emitted in **`version_def.h`**; **must be absent** on **`main`**—run **`make strip-dev-phase-from-ver`** (or **`./scripts/strip_dev_phase_from_ver_trees.sh`**) before merging version files into **`main`** (CI enforces).
