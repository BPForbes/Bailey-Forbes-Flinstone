...
- Each new PR scope, develop iteration, or go-to-main step: **add exactly one new** **`version/entries/A_B_C_slug.ver`** at the **entries root** (for **`PRERELEASE=1`**, CI **`relocate_root_prerelease_ver_to_preproduction.sh`** moves it under **`preproduction <A>.<B>.<C>/`**).
...
On **same-repo** pull requests into **`develop`** and on **feature-branch pushes** (not **`develop`** / **`main`** themselves), GitHub Actions may commit updates for you: **`.github/actions/prepare-version-entries`** runs **`relocate_root_prerelease_ver_to_preproduction.sh`**, then **`stamp_version_entries_release_date.sh`** (entries root **`*.ver`** plus every **`preproduction <A>.<B>.<C>/`** tree on the branch), before **`check_version_prerelease_layout.sh`**. The same workflow job then runs **`./scripts/gen_version_def.sh`** (which already reads **`version/entries/**/*.ver`**, including **`preproduction */`**) and may push one automated commit covering **`version/entries`** and **`userland/shell/version_def.h`** when needed. **Fork** pull requests cannot receive that push—run **`make prepare-version-entries`** (or both scripts) and **`gen_version_def.sh`** locally, commit, and push. **`develop`** uses **Version lock on merge** to relocate before **`finalize_version_locked.sh`** and includes **`version/entries/**`** in the sync PR when paths change.
