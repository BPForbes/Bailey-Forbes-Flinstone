...
**Combining AI/agent branches (one PR / one train):** When merging a side agent branch into the main feature branch, use **`git merge --no-ff <side-branch>`** so the combine point is visible. After the merge, keep **exactly one** **`.ver`** for that **A.B.C** semver train: **keep the file that belonged to the branch you merged into** (same path/basename; often under **`version/entries/preproduction A.B.C/`** after CI relocate). Fold the merged branch’s release prose into that file’s **`DESCRIPTION`**, **delete** the duplicate **`.ver`**, and run **`./scripts/check_version_entries_semver_dev_unique.sh`**. Authoritative checklist: **AGENTS.md** § *AI feature-branch workflow*; IDE rules **`.cursor/rules/versioning.mdc`**; review hints **`.coderabbit.yaml`**.
