...
- The **one new root `.ver` this PR adds** (the file you created at **`version/entries/`**, even after CI moves it under **`preproduction */`**): you may change **`DESCRIPTION` only** until merge to **`develop`** (no **`DEV_VERSION`** re-bump). After merge, that path is immutable.
...
Full rules are in **§3** above. Replit Agent: **one new** root **`version/entries/A_B_C_slug.ver`** per PR train; **`PRERELEASE=1` at root only** (CI moves to **`preproduction */`**); **never** hand-add or hand-edit under **`preproduction */`**; **never** **`GM=1`**; **never** merge-base **`.ver`** edits; **`DESCRIPTION` only** on the new row this PR added; **never** **`version/locked/**`** or **`version_def.h`** commits.
