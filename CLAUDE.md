...
- **No semver backtracking (AI):** If milestone **`4.0.0`** prerelease rows live under **`preproduction 4.0.0/`**, do **not** add a **new root** **`3_3_1_*.ver`** (or any **A.B.C** below that line) unless a **human maintainer** explicitly asks—use **`DEV_VERSION`** or **another `*.ver` in the same preproduction directory** (e.g. **`DEV_VERSION=1`** then **`2`**) to record iterations. For routine **GA** work at **`version/entries/`** root, **one** **`.ver`** updated in place is often enough.
