...
- **`PRERELEASE`** — **`0`** or **`1`**; **`1`** = prerelease row that must end under **`version/entries/preproduction A.B.C/`** (same-repo PR CI may commit the move from the entries root; fork PRs must run relocate locally — see **`docs/versioning.md`**).
...
**AI assistants:** When you begin the **first** substantive code change for a pull request, **add** or extend **`version/entries/*.ver`** (or **`preproduction A.B.C/`** rows) so the work is documented. **Do not backtrack semver against an active preproduction version:** the **preproduction version** is **A.B.C** from directory names **`version/entries/preproduction <A>.<B>.<C>/`** (that is where **`PRERELEASE=1`** rows belong); do **not** add a **new root** **`version/entries/*.ver`** whose **A.B.C** is **numerically below** that **A.B.C** unless a **human maintainer** explicitly asks for that separate shipped release—iterate with **`DEV_VERSION`** or **another `*.ver` in the same preproduction directory**. For a **single GA feature** at **`version/entries/`** root without **`preproduction */`**, **one** **`.ver`** revised in place is often enough. **Do not** commit **`version/locked/**`** or **`userland/shell/version_def.h`** on agent-authored feature PRs (see **Versioning** above).
