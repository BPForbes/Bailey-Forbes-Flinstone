...
"  server host -all <port>                     Listen on all IPv4 interfaces (0.0.0.0)\n"
"  server host :<port>                         Shorthand for server host -all <port>\n"
"  server interfaces                           List local IPv4 addresses for LAN/WLAN hosting\n"
...
"  server file [-all | -user <name> [-id <N>] | -id <N>] <path> [-vweor...] [-Ex<expiry>]\n"
"                                              Offer a file (public -all or private -user/-id)\n"
"  server send -file <path> -user <name> ... Legacy alias for server file -user\n"
"  server file inbox|public|sent               List file offers (inbox / hub public / sent)\n"
"  server file accept <share_id> [--overwrite | --server-share]\n"
"  server file decline <share_id>              Decline a file offer\n"
"  server file revoke <share_id>               Revoke an offer you sent (host/catalog)\n"
"  server file fetch <content_hash>            Fetch blob by SHA-256 content hash\n"
"\n"
"Other:\n"
...
"  ping6 <host>         ICMPv6 echo (loopback ::1 and resolved AAAA targets)\n"
