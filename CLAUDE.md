...
- **Cursor / AI-authored feature PRs:** commit **`version/entries/*.ver`** (and keep **`version/entries/ABOUT.txt`** byte-identical to the merge target’s **`version/locked/ABOUT.txt`** while **`version/locked/`** is unchanged—CI `check_version_locked_subset_of_entries.sh`). **Do not** commit **`version/locked/**`** in those PRs: **Version lock on merge** (GitHub Actions after push to **`develop`**) runs **`relocate_root_prerelease_ver_to_preproduction.sh`**, **`finalize_version_locked.sh`**, stamps dates when applicable, **`gen_version_def.sh`**, and opens a PR to publish **`version/locked/`** and the header when needed. **`userland/shell/version_def.h`**: regenerate with **`./scripts/gen_version_def.sh`** and commit whenever **`.ver`** edits would change **`VERSION_LINE`** (for example **`PRERELEASE`**, **`PRERELEASE_TAG`**, **`DEV_VERSION`**, or which **`PRERELEASE=1`** row wins), or when CI **`check_version_header_matches_locked`** fails; otherwise leave the header to **Version lock on merge** so CI matches the generator. **Maintainers** may run finalize locally before merge if they want an early snapshot on the branch.
