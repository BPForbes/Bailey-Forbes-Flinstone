...
- Each new PR scope, develop iteration, or go-to-main step: **add exactly one new** **`version/entries/A_B_C_slug.ver`** at the **entries root** (for non-GM **`PRERELEASE=1`**, CI **`relocate_root_prerelease_ver_to_preproduction.sh`** moves it under **`preproduction <A>.<B>.<C>/`**).
