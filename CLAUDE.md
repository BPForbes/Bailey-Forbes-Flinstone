...
- **Optional prerelease layout:** **`PRERELEASE=1`** rows live under **`version/entries/preproduction A.B.C/`**; bump **`DEV_VERSION`** with **`./scripts/bump_dev_version.sh`**. Set **`GM=1`** on exactly one file when the line is ready, then run **`./scripts/promote_preproduction_for_main.sh`** (or **`make promote-preproduction-for-main`**) so every **`*.ver`** in that folder is merged into one root GA row and the **`preproduction *`** directory is deleted from **both** **`version/entries`** and **`version/locked`**. Before any merge that lands **`version/**`** on **`main`**, ensure that promotion has already run — **`main`** must not ship **`preproduction *`**, **`PRERELEASE=1`**, **`GM=1`**, or **`DEV_VERSION=`** — CI fails otherwise.
