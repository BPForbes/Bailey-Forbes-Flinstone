...
**AI assistants:** When you begin the **first** substantive code change for a pull request, **add** or extend **`version/entries/*.ver`** (or **`preproduction A.B.C/`** rows) so the work is documented. **Do not backtrack semver for in-flight milestone work:** if **`4.0.0`** prerelease rows live under **`preproduction 4.0.0/`**, do **not** add a **new root** **`3_3_1_*.ver`** (or any **A.B.C** below that milestone) unless a **human maintainer** explicitly asks for that separate shipped release—iterate with **`DEV_VERSION`** or **another `*.ver` in the same preproduction directory** (e.g. **`DEV_VERSION=1`** then **`DEV_VERSION=2`**). For a **single GA feature** at **`version/entries/`** root without preproduction, **one** **`.ver`** revised in place is often enough. **Do not** commit **`version/locked/**`** or **`userland/shell/version_def.h`** on agent-authored feature PRs (see **Versioning** above).
