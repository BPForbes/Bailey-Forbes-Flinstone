...
Implement the bump by adding **`version/entries/<A>_<B>_<C>_<slug>.ver`**. Pushing to **`develop`** triggers **Version lock on merge** (entries → **`version/locked/`**, regenerate **`version_def.h`**, pushes a **sync branch**—open PR from Actions summary compare link). For a local release build after that, use **`make deploy`** or the **Deploy** workflow.
