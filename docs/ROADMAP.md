...
**P3 row criterion (aligned with `contracts/networking/`):** **P3-1** through **P3-14** are **✅** here when the normative **C contract bundle** under **`contracts/networking/`** defines that row via **`contract_networking.h`** (umbrella: **`contract_extend.h`** + **`contract_p3_wire.h`** + **`contract_p3_packet.h`** (layered packet/pipeline vocabulary, not a **P3-*** row) + **`contract_p3_trust.h`**, then **`contract_p3_*.h`** shards with **`FL_CONTRACT_P3_*_CONTRACT_DEFINED`** markers). **`contract_p3_trust.h`** composes the **P2-3** `fl_authz_operation_t` slice only so **P3** is **not** an include-graph clone of **`contract_identity.h`**. **P3-10** / **P3-11** shards record explicit **`[DEFERRED]`** scope at the **contract-definition** layer. **Phase 3** implementation (stack, drivers, CI interop) still follows phase gates below; the **Module integration** column tracks **`kernel/core/net/`**, **`userland/command/`** (shell verbs), **`tools/FlinstonePowershell/`** (WSL→Windows bridge), and hosted shims on **`develop`** / **PRE 4.2.0** trains — file-level map: **`docs/P3_NETWORKING.md`**. **Snapshot (PRE 4.2.0 / 4.3.0):** every **P3-*** row is **contract ✅**; **integration ~✅** except **P3-10** (**✅**, **#328** / **P4-01** closed). **~✅** = usable **H-track** lab subset, not bare-metal/native-NIC production (except **P3-10**, which has maintainer-confirmed physical FullMAC evidence). The **P3 → P4** gate (**P3-2** loopback + **P3-6** UDP stable; **P3-3** TAP CI policy) is **met**; remaining tails (**#238**, **#280**, native **`fl_socket`**, **P5** file delivery) are documented deferrals, not missing rows. Legend: **✅** shipped to phase acceptance; **~✅** usable lab subset (shell **`ping`** / **`check requirements`**, CUnit, documented skips); **⚠️** started but below row goals; **❌** no in-tree implementation.
...
**P8 row criterion (aligned with `contracts/virtualization/`):** **P8-1** through **P8-3** are **✅** for **contract completion** when the normative **C contract bundle** under **`contracts/virtualization/`** defines those rows via **`contract_virtualization.h`** (**`FL_CONTRACT_P8_VIRTUALIZATION_REV` 2**; umbrella: **`contract_extend.h`**, then **`contract_p8_timing.h`**, **`contract_p8_virtio_guest.h`**, **`contract_p8_qemu_lab.h`**, and **`contract_p8_browser_artifact.h`** with **`FL_CONTRACT_P8_*_CONTRACT_DEFINED`** markers). **P4-4** virtio queue mechanics remain **`contracts/drivers/`**; include **`contract_drivers.h`** only where a translation unit programs rings. **Module integration** stays **❌** in this snapshot until **Phase 8** gates land (e.g. **NIC** replay in **`make test_replay`**, inter-VM automation beyond static docs, **VM** parity on metal where claimed). **`docs/qemu-lab.md`**, **`docs/browser-kernel-lab.md`**, **`contracts json`** (**`p8_virtualization_rev`** and **`p8_browser_artifact_rev`**), **`make test_invariants`**, and **`scripts/baseline_tests.sh`** are **contract-packaging / verification** aids—they do **not** finalize the integration column.
...
| **P8-2** | Guest virtio | ✅ | ❌ |
| **P8-3** | QEMU / machine emulation and browser artifact (lab) | ✅ | ❌ |
| **P9-1** | Fuzzing | ✅ | ❌ |
| **P9-2** | Coverity / static analysis | ✅ | ❌ |
| **P9-3** | SMP bring-up (B) | ✅ | ❌ |
| **P9-4** | RCU grace-period jobs | ✅ | ❌ |

**Summary:** **Contract completion** — **P0-1**–**P0-8**, **P1-1**–**P1-7**, **P2-1**–**P2-4**, **P3-1**–**P3-14** (including **`[DEFERRED]`** shards), **P4-1**–**P4-7**, **P5-1**–**P5-7**, **P6-1**–**P6-5**, **P7-1**–**P7-3**, **P7 (batch)**, **P8-1**–**P8-3**, and **P9-1**–**P9-3** are **✅** under their **`contracts/*`** bundles. **Module integration (P0–P2)** — **✅:** **P0-1**–**P0-3**, **P0-6**–**P0-8**, **P1-1**, **P2-1**, **P2-2**, **P2-4** (hosted/lab wired and tested on **4.1.0**). **~✅:** **P1-7** (hosted POSIX `clock_gettime` only; **B** Generic Timer / **P0-5** tick follow-up). **~✅:** **P0-4**, **P0-5** (arch **B** evidence TODOs), **P1-2**–**P1-6** (**B** PMM/arena/NASM), **P2-3** (guest deny suite + shell gates; netdev hook on **PRE 4.2.0** **`ping`** path—see **TODO: P2-3**). See **`docs/p0_p2_pr_coverage.md`**. **Module integration (P3, PRE 4.2.0 / 4.3.0)** — **✅:** **P3-10** (**#328** / **P4-01** closed). **~✅:** **P3-1**–**P3-9**, **P3-11**–**P3-14** (in-tree stack under **`kernel/core/net/`** + **`fl_net_packet_t`** layering, hosted socket/TCP stream shim, DHCP codec/lab client, ARP sweep on workqueue, TLS sizing hook, **server** foundations from `#239` / PR `#282` — `server host/join/leave/kill/announce/msg/nick` + `udpsend` / `udplisten` + `arp` / `ifconfig` / `route` / `netstat` / `nslookup` / `netsh` distro-style verbs; **PR #315** WSL LAN portproxy/firewall UX via **`FlinstonePowershell.exe`** + **`net_wifi_host_linux.c`**, UAC press-any-key gate, exit teardown, faster netsh apply; **`make test_p3_network`** / `make test_p3_server` / `make test_p3_udp_cmds` / `make test_p3_net_tools`; TAP/interop skips documented). Deferred siblings tracked: `#283` (`OP_CTRL_HOST_PROMOTE6`), `#280` (IPv6 wire/TCPv6), native non-hosted `fl_socket` (gated on **P3-7**) — see **`docs/P3_13_FOLLOWUP.md`**. **P3-10** module integration **✅** (**#328** / **P4-01** closed: physical ESP UART scan/join, real-AP TWT `flow_id`, in-tree DHCP/UDP on a Linux 802.11ax FullMAC adapter). **P3-11** module integration **~✅** foundation (**#280** PR #301). Detail: **`docs/P3_NETWORKING.md`**, **`docs/GITHUB_ISSUE_SYNC_P4_WIFI_OTA.md`**. **P4-1**/**P4-2** **✅**; **P8**/**P9** integration **❌** here.
...
| **A2** | **P1** + **P2** + **P3** through **P3-6** (UDP) with loopback + TAP path | **P1-4**/**P1-5** PMM/arenas validated on **B** where applicable; **P2-3** authz tests deny guest on privileged ops; UDP/loopback interop tests in CI or documented skip. **PRE 4.2.0** progress: loopback + ARP + LPM + hosted UDP/ICMP/DNS + DHCP codec (**~✅**); **P3-13** **`server`** app **~✅** (PR #282 foundations + #239 `udpsend`/`udplisten` + PR #315 WSL portproxy/UAC hosting UX) |
...
| **P3 → P4** | **P3-2** loopback + **P3-6** UDP stable; TAP (**P3-3**) policy documented for CI — **satisfied on PRE 4.2.0 / develop** (native NIC / production Wi‑Fi / bare-metal wire proof remain **P4**-class) |
...
| **P3-13** | ~✅ | Server foundations (**#239** / PR #282) plus PR #301: channel sidecar, host catalog, **#283** PROMOTE6 + `host_addr` callback, **#280** IPv6 loopback foundation, **#284** endian norm. **#279** Wi‑Fi **~✅** foundation (PR #306). **PR #315:** WSL LAN exposure via **`net_wifi_host_linux_server_proxy*`** + **`FlinstonePowershell.exe`** (`server-proxy`, `server-proxy-del`, `server-proxy-check`); UAC press-any-key gate + deferred hosting line in **`cmd_server.c`**; portproxy teardown on **`server kill`/`leave`/`exit`** + **`atexit(cmd_server_atexit)`**; interactive Ctrl+C. Native `fl_socket` gated on **P3-7** TCP. **`docs/P3_13_FOLLOWUP.md`**, **`docs/SERVER.md`**. |
| **P3-14** | ~✅ | **`net_background.c`**: **`fl_net_arp_tick`**; RX dequeue / TCP timer wheel remain future |
| **P3-10** | ✅ | **`contract_p3_wifi.h`**, **`net_wifi_he`**, in-tree SAE/EAPOL/TWT OTA (**`wifi_connect_ota`**, **`wifi_twt_ota`**, **`test_wifi_connect_ota`**), **`fl_net_dhcp_acquire`** post-association on Wi‑Fi netdev, lab scan/connect (**`FL_NET_WIFI_LAB`**, **`LabWpa2`** WPA2-PSK without OS supplicant), physical nl80211 path (**`FL_NET_WIFI_IFACE`**), **`wifi_router`** DB + shell **`wifi`** verbs; ESP AT **`wifi scan` / `wifi join`** (**`test_wifi_uart_at_scan_join`** + physical `/dev/ttyUSB*`). **P4-01** (**#328**) **closed:** maintainer-confirmed physical ESP UART scan/join, real-AP TWT Individual Setup/Teardown with negotiated `flow_id`, and in-tree DHCP + UDP echo on a Linux 802.11ax FullMAC adapter (no OS supplicant / OS DHCP client). Issue-specific GH Actions job and **`validate_issue_328.sh`** are sunset. |
| **P3-11** | ~✅ | **`contract_p3_ipv6.h`**, **`net_ipv6`/`net_icmpv6`/`net_ndp`**, loopback ICMPv6 + NDP; TAP/wire egress IPv6 stretch remains |

**Shell / CI:** **`ping`**, **`check requirements`**; **`make test_p3_network`**, **`make test_p3_server`**, **`make baremetal`**, **`make test_invariants`**, **`make test_core`**, **`make check-network-requirements`**. **WSL helper:** **`make flinstone-ps-windows`** builds **`tools/FlinstonePowershell/FlinstonePowershell.exe`**. **ASM:** **`arch/*/net_asm.*`**, **`arch/*/net_wire_host_asm.*`**. **Implementation map:** contracts **`contracts/networking/`**; stack **`kernel/core/net/`**; shell verbs **`userland/command/cmd_{ping,ping6,udp,wifi,server}*.c`**; tests **`tests/test_p3_*.c`**. **PRE 4.2.0 (this train):** lab bare-metal **802.3** path (**`net_baremetal.c`**, **#237** / **#241** / **#240**); umbrella **#238–#267** (excl. **#239** **`server`**) on PR **#275**; production virtio NIC is **P4**-class follow-up.
...
| **TODO: P3-13 (PR #315)** | ✅ **Shipped:** WSL LAN portproxy + firewall via **`FlinstonePowershell.exe`** / **`net_wifi_host_linux_server_proxy*`**; UAC press-any-key gate + deferred **`hosting as`** line; netsh-only elevated **`cmd.exe`** apply/delete; **`server-proxy-check`** preflight; portproxy teardown on **`server kill`/`leave`/`exit`** + **`atexit(cmd_server_atexit)`**; interactive Ctrl+C (NOTICE gate re-prompt). See **`docs/SERVER.md`**, **`docs/P3_NETWORKING.md`**. |
...
| ~~**TODO: P3-13 (#279) / P4-01 (#328)**~~ | ✅ **Closed:** Wi‑Fi 802.11ax station production tail (**P3-10** / **P4-01** / **#328**). Software/CI/lab evidence plus maintainer-confirmed physical ESP UART `wifi scan`/`wifi join`, real-AP TWT Individual Setup/Teardown with negotiated `flow_id`, and in-tree DHCP + UDP echo on a Linux 802.11ax FullMAC adapter. ROADMAP P3-10 module integration is **✅**. See **`docs/GITHUB_ISSUE_SYNC_P4_WIFI_OTA.md`**. |
...
| **TODO: P3-13 (promote-mutex)** | **Partial (PR #315):** **`session_mutex`** in **`cmd_server.c`** serializes host/kill/leave/atexit and releases before UAC keypress waits. **`promote_thread_main`** and full verb audit still open — avoid deadlocks against **`shell_io_lock`** and blocking I/O; see **`docs/P3_13_FOLLOWUP.md` → Deferred items**. |
...
| **P8-3** | **QEMU emulation bring-up** | One-command **QEMU** recipes for CI and developers (disk + netdev + serial). | **Primary reference:** **`docs/qemu-lab.md`** (machine tokens, **`-icount`**, virtio **`-drive`**, **`-netdev`**, TAP bridge). **`docs/`** or **`README`** link that file from the top-level **README**; **machine profile** strings respect **`FL_CONTRACT_P8_QEMU_MACHINE_NAME_MAX_CHARS`** in **`contract_p8_qemu_lab.h`**. Browser candidates use the schema and fail-closed promotion rules in **`contract_p8_browser_artifact.h`**. |

**Module-contract snapshot:** **P8-1**–**P8-3** are **✅** for **contract completion** (**`contracts/virtualization/`**, **`FL_CONTRACT_P8_VIRTUALIZATION_REV` 2**). See the [P0–P9 snapshot](#p0p9-module-contract-snapshot-develop).
...
| Wi‑Fi | **IEEE 802.11ax-2021** / **802.11i**; **WPA3-SAE**; **RFC 2131** (DHCP after link). **P3-10** contract **✅**; module **✅** — in-tree OTA plus maintainer-confirmed physical UART / real-AP TWT / FullMAC DHCP+UDP (**#328** / **P4-01** closed). **`docs/GITHUB_ISSUE_SYNC_P4_WIFI_OTA.md`**. |
